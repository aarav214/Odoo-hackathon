...
export default function DocumentsCard() {
  return (
    <div className="bg-white rounded-3xl border border-[#EDE8E0] warm-shadow card-hover p-6 fade-in-up">
        ...
        {docs.map(({ name, date, icon: Icon, color }) => (
          ...
          >
            ...
            <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              <button className="w-8 h-8 rounded-lg bg-[#F4EFE7] hover:bg-[#EDE8E0] flex items-center justify-center transition-colors">
                <Eye size={13} className="text-[#6E675F]" />
              </button>
              <button className="w-8 h-8 rounded-lg bg-[#7BAE7F15] hover:bg-[#7BAE7F25] flex items-center justify-center transition-colors">
      ...
      <button className="mt-4 w-full flex items-center justify-center gap-2 bg-gradient-to-r from-[#7BAE7F15] to-[#A98E7415] hover:from-[#7BAE7F25] hover:to-[#A98E7425] border border-dashed border-[#A98E7450] text-[#5A9260] text-sm font-medium py-3 rounded-2xl btn-scale">
