    ...
    <aside className="fixed left-0 top-0 h-full w-[220px] bg-white border-r border-[#EDE8E0] flex flex-col z-20 warm-shadow">
        ...
        {navItems.map(({ icon: Icon, label }) => (
          ...
          >
            <div className={`w-8 h-8 rounded-lg flex items-center justify-center transition-colors ${
              active === label ? 'bg-[#7BAE7F20]' : 'group-hover:bg-[#EDE8E0]'
            }`}>
              <Icon size={16} className={active === label ? 'text-[#7BAE7F]' : ''} />
            </div>
            <span>{label}</span>
            {active === label && (
      ...
      <div className="px-3 pb-6 space-y-1">
        <div className="mx-3 mb-4 p-3 rounded-xl bg-gradient-to-br from-[#7BAE7F15] to-[#A98E7415] border border-[#EDE8E0]">
          ...
          <button className="mt-2 text-[11px] font-medium text-[#7BAE7F] hover:underline">Open ticket</button>
        </div>
        <button className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium text-[#6E675F] hover:bg-[#F4EFE7] hover:text-[#2F2A26] transition-all">
