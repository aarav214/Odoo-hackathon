...
import { ChevronLeft, ChevronRight, Calendar as CalendarIcon, User, Plus, Clock, Check, AlertCircle, X } from 'lucide-react';
        ...
        <div className="fixed inset-0 bg-[#2d2a2650] backdrop-blur-sm flex items-center justify-center z-[100] p-4">
          <div className="bg-white rounded-3xl p-6 w-full max-w-sm border border-[#EDE8E0] shadow-2xl">
            <div className="flex items-center justify-between mb-4">
              <h4 className="text-[#2F2A26] font-bold text-base">
                Schedule for July {selectedDayEvents.day}, {year}
              </h4>
              <button
                onClick={() => setSelectedDayEvents(null)}
                className="text-[#6E675F] hover:text-[#2F2A26] p-1"
              >
                <X size={16} />
              </button>
            </div>
            ...
            <div className="space-y-3 my-4">
              {selectedDayEvents.events.length === 0 ? (
                <p className="text-xs text-[#6E675F] italic">No events or leaves scheduled for this day.</p>
              ) : (
                selectedDayEvents.events.map((e, idx) => (
                  <div key={idx} className="p-3 rounded-2xl border border-[#EDE8E0] bg-[#FAF8F4] flex flex-col gap-1 text-left">
                    <div className="flex items-center gap-2">
                      <span className="w-2.5 h-2.5 rounded-full shrink-0" style={{ background: e.color }} />
                      <span className="text-xs font-bold text-[#2F2A26]">{e.title}</span>
                    </div>
                    {e.description && (
                      <p className="text-[10px] text-[#6E675F] pl-4.5">{e.description}</p>
                    )}
                    {e.status && (
                      <span className="text-[9px] font-semibold px-2 py-0.5 rounded-full mt-1.5 self-start uppercase tracking-wider bg-[#EDE8E0] text-[#6E675F]">
                        Status: {e.status}
                      </span>
                    )}
                  </div>
            ...
            <button
              onClick={() => setSelectedDayEvents(null)}
              className="w-full bg-[#7BAE7F] hover:bg-[#5A9260] text-white text-xs font-semibold py-2.5 rounded-xl mt-2"
            >
              Close
            </button>
          </div>
