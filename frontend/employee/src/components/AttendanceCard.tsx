...
export default function AttendanceCard({ onTabChange }: { onTabChange?: (tab: string) => void }) {
  ...
  const [timeline, setTimeline] = useState<any[]>(weekDays);
  const [presentDaysCount, setPresentDaysCount] = useState(4);
    ...
    async function loadAttendance() {
      try {
        ...
        if (isOnline) {
          ...
          // Dynamically map weekly timeline
          const days = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
          const todayObj = new Date();
          const dayOffset = todayObj.getDay() === 0 ? -6 : 1 - todayObj.getDay();
          const mondayDate = new Date(todayObj.setDate(todayObj.getDate() + dayOffset));
          
          let presentCount = 0;
          const updatedTimeline = days.map((dayName, idx) => {
            const dateForDay = new Date(mondayDate);
            dateForDay.setDate(mondayDate.getDate() + idx);
            const dateStr = `${dateForDay.getFullYear()}-${String(dateForDay.getMonth() + 1).padStart(2, '0')}-${String(dateForDay.getDate()).padStart(2, '0')}`;
            
            const recordForDay = records.find(r => r.date && r.date.startsWith(dateStr));
            
            let status = 'absent';
            let hours = '-';
            
            const isFuture = dateForDay.setHours(0,0,0,0) > new Date().setHours(0,0,0,0);
            ...
            if (isWeekend) {
              ...
              status = 'present';
              presentCount++;
              if (recordForDay.check_in_time && recordForDay.check_out_time) {
                const diffMs = new Date(recordForDay.check_out_time).getTime() - new Date(recordForDay.check_in_time).getTime();
                const diffHrs = Math.floor(diffMs / 3600000);
                const diffMins = Math.floor((diffMs % 3600000) / 60000);
                hours = `${diffHrs}h ${diffMins}m`;
              } else if (recordForDay.check_in_time) {
                hours = 'Active';
              } else {
                hours = '8h 00m';
              }
            ...
            return { day: dayName, status, hours };
          });
    ...
    <div className="bg-white rounded-3xl border border-[#EDE8E0] warm-shadow card-hover p-6 fade-in-up">
      ...
      <div className="mt-5 pt-5 border-t border-[#EDE8E0]">
        <div className="flex items-center justify-between mb-3">
          ...
          <span className="text-[11px] text-[#6E675F]">{presentDaysCount}/5 days present</span>
        </div>
        <div className="flex items-center justify-between">
          {timeline.map(({ day, status, hours }) => (
