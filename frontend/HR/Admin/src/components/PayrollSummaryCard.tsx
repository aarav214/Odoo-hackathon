...
import { FileText, Download, FileSpreadsheet, Wallet, Calendar, Loader2 } from 'lucide-react';
...
export default function PayrollSummaryCard() {
  ...
  const handleAction = (type: string) => {
    ...
    setTimeout(() => {
      ...
      if (type === 'generate') {
        alert('Payroll calculations completed successfully. All ledger sheets are up to date!');
      } else if (type === 'pdf') {
        alert('Payroll report PDF generated and downloaded to your computer.');
      } else if (type === 'excel') {
        alert('Payroll report spreadsheet (.xlsx) exported and downloaded successfully.');
      }
    }, 1200);
  };
    ...
    <div className="card p-5 flex flex-col" style={{ borderRadius: 24, height: 300 }}>
      ...
      <div className="flex gap-1.5 mt-auto">
        <button
          ...
          className="btn-primary flex-1 justify-center text-[11px] py-2"
        >
          ...
          <span>{loadingAction === 'generate' ? 'Working...' : 'Generate'}</span>
        </button>
        <button
          onClick={() => handleAction('pdf')}
          disabled={loadingAction !== null}
          className="btn-secondary flex-1 justify-center text-[11px] py-2"
        >
          {loadingAction === 'pdf' ? (
            <Loader2 size={12} className="animate-spin" />
          ) : (
