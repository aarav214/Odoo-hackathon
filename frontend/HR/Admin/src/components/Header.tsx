...
interface HeaderProps {
  ...
  onAddEmployee?: () => void;
  onSearchSelect?: (category: string, item: string) => void;
}

export default function Header({ onLogout, adminName = 'Admin User', activeTab = 'Dashboard', onTabChange, onAddEmployee, onSearchSelect }: HeaderProps) {
  ...
  return (
    <header className="h-[72px] bg-white border-b flex items-center px-7 gap-6 sticky top-0 z-50" style={{ borderColor: '#E8DFD3', boxShadow: '0 1px 3px rgba(45,36,25,0.04)' }}>
      ...
      {/* Add Employee */}
      <button onClick={onAddEmployee} className="btn-primary gap-1.5">
              ...
              <input
                ...
                onKeyDown={e => {
                  if (e.key === 'Enter' && searchQuery) {
                    onSearchSelect && onSearchSelect('Employees', searchQuery);
                    setShowSearch(false);
                  }
                }}
              ...
              <div key={cat.label} className="mb-3">
                ...
                {cat.results.map(r => (
                  <button
                    key={r}
                    onClick={() => {
                      onSearchSelect && onSearchSelect(cat.label, r);
                      setShowSearch(false);
                    }}
                    className="w-full text-left px-3 py-2 rounded-xl text-[13px] hover:bg-[#F5EDE0] transition-all"
                    style={{ color: '#8A7B6A' }}
                  >
                    {r}
                  </button>
